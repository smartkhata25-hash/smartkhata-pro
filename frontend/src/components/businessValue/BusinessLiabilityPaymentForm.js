...
const BusinessLiabilityPaymentForm = ({ isOpen = false, liability = null, onClose, onPaid }) => {
  ...
  useEffect(() => {
    ...
    const loadAccounts = async () => {
      try {
        ...
        console.error('Liability Payment Accounts Error:', loadError);

        if (active) {
          setAccounts([]);
          setError(t('businessValue.paymentAccountsLoadFailed'));
        }
      } finally {
        if (active) {
  ...
  useEffect(() => {
    ...
    const currentAccountStillValid = filteredAccounts.some(
  ...
  }, [filteredAccounts, form.accountId, form.paymentMethod, isOpen]);
  ...
  const validateForm = () => {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      return t('businessValue.paymentAmountRequired');
    }

    if (amount > remainingAmount) {
      return t('businessValue.paymentExceedsRemaining');
    }

    if (!form.paymentDate) {
      return t('businessValue.paymentDateRequired');
    }

    if (!form.accountId) {
      return t('businessValue.paymentAccountRequired');
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!liability?._id || saving) return;
    ...
    try {
      ...
      const result = await payBusinessLiability(liability._id, {
      ...
      console.error('Liability Payment Submit Error:', submitError);

      setError(submitError.message || t('businessValue.paymentFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm sm:p-4">
      ...
      <form
        ...
        className="
          relative z-10 flex max-h-[92vh] w-full
          max-w-[620px] flex-col overflow-hidden
          rounded-3xl border border-white/60
          bg-white shadow-2xl
        "
      >
        <div className="relative overflow-hidden bg-gradient-to-r from-indigo-700 via-blue-700 to-cyan-600 px-4 py-3.5 text-white sm:px-5">
          <div className="absolute -right-8 -top-12 h-28 w-28 rounded-full bg-white/10 blur-xl" />
          ...
          <div className="space-y-3 p-3 sm:p-5">
            ...
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              <div className="relative">
                <input
                  ...
                  placeholder={t('businessValue.paymentAmountPlaceholder')}
                  disabled={saving}
                  className="
                    h-11 w-full rounded-xl
                    border border-slate-200
                    bg-slate-50 px-3 pr-16
            ...
            <div className="relative">
              ...
              >
                <option value="">
                  {accountsLoading
                    ? t('businessValue.loadingPaymentAccounts')
                    : t('businessValue.selectPaymentAccount')}
                </option>
            ...
            {!accountsLoading && filteredAccounts.length === 0 && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                {t('businessValue.noPaymentAccountForMethod')}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              <input
                ...
                maxLength={100}
                disabled={saving}
                placeholder={t('businessValue.paymentReferencePlaceholder')}
                className="
              ...
              <input
                ...
                maxLength={500}
                disabled={saving}
                placeholder={t('businessValue.paymentNotePlaceholder')}
                className="
                  h-11 rounded-xl border
                  border-slate-200 bg-slate-50
                  px-3 text-sm text-slate-700
            ...
            <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50 px-3 py-2">
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs font-semibold text-slate-500">
                  {t('businessValue.remainingAfterPayment')}
                </span>

                <span className="text-sm font-black text-indigo-700">
                  {t('currency.rs')}{' '}
                  {formatLiabilityAmount(Math.max(remainingAmount - Number(form.amount || 0), 0))}
                </span>
              </div>
            </div>
          </div>
          ...
          >
            {saving ? (
              <>
                ...
                <span>{t('businessValue.processingPayment')}</span>
              </>
            ) : (
              <>
                <span>✓</span>
                <span>{t('businessValue.payNow')}</span>
              </>
            )}
          </button>
        </div>
...
const AmountStat = ({ label, value, styleType = 'neutral' }) => {
  const styles = {
  ...
  return (
    <div
      className={`
        ...
        px-2.5 py-2
        ${styles[styleType] || styles.neutral}
      `}
    >
...
export default BusinessLiabilityPaymentForm;
