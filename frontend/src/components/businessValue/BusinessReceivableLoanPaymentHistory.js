...
const BusinessReceivableLoanPaymentHistory = ({
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      console.error('Receivable Loan Payment History Load Error:', loadError);

      setError(loadError.message || t('businessValue.receivableLoanHistoryLoadFailed'));
    } finally {
      setLoading(false);
    }
  }, [loanId, moduleScope]);
  ...
  const handleReverse = async (payment) => {
    ...
    const confirmed = window.confirm(t('businessValue.receivableLoanReverseConfirm'));
    ...
    try {
      ...
      console.error('Reverse Receivable Loan Repayment Error:', reverseError);

      setError(reverseError.message || t('businessValue.receivableLoanReverseFailed'));
    } finally {
      setReversingId('');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm sm:p-4">
      ...
      <div
        ...
        "
      >
        <div className="relative overflow-hidden bg-gradient-to-r from-emerald-800 via-teal-800 to-cyan-700 px-4 py-3.5 text-white sm:px-5">
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2 className="truncate text-base font-black sm:text-lg">
                  {t('businessValue.receivableLoanPaymentHistory')}
                </h2>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-slate-100 bg-white px-3 py-3 sm:px-5">
          <div className="text-[11px] font-semibold text-slate-400">
            {t('businessValue.receivableLoanHistoryNote')}
          </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <div className="text-sm font-black text-slate-800">
                {t('currency.rs')} {formatReceivableLoanAmount(payment.amount)}
              </div>
          ...
          <div className="mt-2 grid grid-cols-1 gap-1.5 text-xs text-slate-500 sm:grid-cols-2">
            <DetailLine
              label={t('businessValue.receivedIntoAccount')}
            ...
            <DetailLine
              ...
              value={`${t('currency.rs')} ${formatReceivableLoanAmount(payment.remainingBefore)}`}
            />

            <DetailLine
              label={t('businessValue.remainingAfter')}
              value={`${t('currency.rs')} ${formatReceivableLoanAmount(payment.remainingAfter)}`}
            />
          </div>
              ...
              >
                {reversing ? (
                  ...
                  <>
                    ...
                    <span>{t('businessValue.reverseReceivableLoanPayment')}</span>
                  </>
                )}
              </button>
            </div>
...
const SummaryCard = ({ label, value, type = 'currency', variant = 'neutral' }) => {
  ...
  if (type === 'currency') {
    displayValue = `${t('currency.rs')} ${formatReceivableLoanAmount(value)}`;
  }
  ...
  return (
    <div className="flex min-h-[240px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
      ...
      <h3 className="mt-3 text-sm font-black text-slate-700">
        {t('businessValue.noReceivableLoanPaymentHistory')}
      </h3>

      <p className="mt-1 max-w-sm text-xs leading-relaxed text-slate-400">
        {t('businessValue.noReceivableLoanPaymentHistoryDescription')}
      </p>
    </div>
  );
};

export default BusinessReceivableLoanPaymentHistory;
